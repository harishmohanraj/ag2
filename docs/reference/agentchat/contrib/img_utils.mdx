...
```python
get_image_data(image_file: str | PIL.Image.Image, use_b64=True) -> bytes
```
...
| Name | Description |
|--|--|
| `image_file` | **Type:** `str \| PIL.Image.Image` |
| `use_b64=True` |  |
...
```python
get_pil_image(image_file: str | PIL.Image.Image) -> PIL.Image.Image
```
...
```python
num_tokens_from_gpt_image(
    image_data: str | PIL.Image.Image,
...
| Name | Description |
|--|--|
| `image_data` | Union[str, Image.Image]: The image data which can either be a base64 encoded string, a URL, a file path, or a PIL Image object.<br/><br/>**Type:** `str \| PIL.Image.Image` |
...
```python
pil_to_data_uri(image: PIL.Image.Image) -> str
```
