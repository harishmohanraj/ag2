...
bind(
    self,
    agent: autogen.agentchat.assistant_agent.AssistantAgent,
    functions: str
) -> 
```
...
| Name | Description |
|--|--|
| `agent` | **Type:** `autogen.agentchat.assistant_agent.AssistantAgent` |
| `functions` | **Type:** `str` |
...
bind_user_proxy(
    self,
    agent: autogen.agentchat.user_proxy_agent.UserProxyAgent,
    tool_root: str | list
) -> 
```
