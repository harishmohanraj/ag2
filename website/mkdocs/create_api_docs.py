...
def _get_submodules(package_name: str) -> List[str]:
    ...
    try:
        ...
        raise e
    submodules = [info.name for info in walk_packages(m.__path__, prefix=f"{package_name}.")]
    submodules = [x for x in submodules if not any(name.startswith("_") for name in x.split("."))]
    return [package_name, *submodules]


def _import_submodules(module_name: str, include_public_api_only: bool = False) -> Optional[List[ModuleType]]:
...
def _import_all_members(module_name: str, include_public_api_only: bool = False) -> List[str]:
    submodules = _import_submodules(module_name, include_public_api_only=include_public_api_only)
    members: List[Tuple[str, Union[FunctionType, Type[Any]]]] = list(
        itertools.chain(*[
            _import_functions_and_classes(m, include_public_api_only=include_public_api_only) for m in submodules
        ])
    )

    names = [y if isinstance(y, str) else f"{y.__module__}.{y.__name__}" for x, y in members]
    names = [name for name in names if not _is_private(name) and name.startswith(module_name)]
    return names
...
def _get_submodule_members(module_name: str) -> List[str]:
    ...
    members_with_submodules_str: List[str] = [x[:-1] if x.endswith(".") else x for x in members_with_submodules]
