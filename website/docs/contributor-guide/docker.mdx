...
```bash
docker run -it -p 8081:3000 -v $(pwd):/home/autogen/ag2 ag2_dev_img bash
```

- Note that the `pwd` is shorthand for present working directory. Thus, any path after the pwd is relative to that. If you want a more verbose method you could remove the "$(pwd)" and replace it with the full path to your directory
